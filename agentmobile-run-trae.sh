#!/bin/bash
# agentmobile-run-trae.sh — 以指定配置 profile 启动 Trae CLI
# 用法: agentmobile-run-trae.sh <profile_id> <project_absolute_path>
...
if [ -z "$PROJECT" ]; then
    echo "[agentmobile] Usage: agentmobile-run-trae.sh <profile> <project_path>"
    exit 1
fi
...
DEFAULT_MODEL="${DEFAULT_MODEL:-${TRAE_MODEL:-}}"
...
if agentmobile_has_profile "$PROFILE"; then
    unset OPENAI_BASE_URL OPENAI_API_KEY TRAE_MODEL
fi
...
if [ -n "$DEFAULT_MODEL" ]; then
    export TRAE_MODEL="$DEFAULT_MODEL"
fi
...
TRAE_BIN="$(agentmobile_resolve_binary trae-cli trae)" || agentmobile_cli_error_and_shell "Trae CLI" "trae-cli trae"

echo ""
echo "╔══════════════════════════════════════════╗"
echo "║  agentmobile · Trae CLI Session"
...
while true; do
    "$TRAE_BIN" interactive || true
    agentmobile_exit_menu "Trae CLI"
done
